# Demo_MIMO_4x8_ChannelRank_TrainSNR_LDPC_v2.py
# --------------------------------------------------
# 4x8 MIMO-OFDM with nonlinear PA + block fading
# ESN-based time-domain recovery vs. LS-ZF / MMSE / Perfect ZF
# LDPC (pyldpc) encoding & decoding for post-LDPC (coded) BER.
# v2 adds:
#   - Higher / adaptive LDPC iterations at low SNR
#   - Empirical sigma^2 per stream for LLRs (decision-directed)
#   - LLR clipping to stabilize decoding
# Saves & shows both pre- and post-LDPC BER curves.
#
# Requirements:
#   pip install pyESN pyldpc scipy matplotlib
#   and ensure helper_mimo_esn_generic.py is in PYTHONPATH
# --------------------------------------------------
...
def unit_qam_constellation(Bi):
    """Square QAM constellation normalized to unit average power, Bi bits/symbol."""
...
def bits_to_grayvec(idx, m):
    """Return LSB-first bit vector for given index (as used in original code)."""
...
def qam_bit_labels(M, m):
    """Labels for each constellation point (consistent with bits_to_grayvec)."""
    labels = np.zeros((M, m), dtype=int)
    for idx in range(M):
        labels[idx, :] = bits_to_grayvec(idx, m)
    return labels  # shape: [M, m]


def qam_llrs_maxlog(z_hat_1d, const, bit_labels, sigma2):
    """
    Max-log LLRs for square QAM.
    z_hat_1d: (N,) complex estimates for one TX across subcarriers
    const:    (M,) complex constellation points
    bit_labels: (M,m) binary labels per constellation index (LSB-first)
    sigma2:   effective noise variance per complex symbol
    returns:  (N*m,) LLRs in order [b0,b1,...] per subcarrier (LSB first)
    """
    N = z_hat_1d.shape[0]
    M = const.shape[0]
    ...
    for i in range(N):
        d2 = np.abs(z_hat_1d[i] - const)**2  # (M,)
        for b in range(m):
            d0 = d2[masks0[b]]
            d1 = d2[masks1[b]]
...
def est_sigma2_from_decision(Xhat_col, const):
    """Decision-directed residual variance estimate (per stream)."""
...
def ldpc_encode_bits(G, u):
    """Encode info bits u using generator matrix G (works for sparse or dense)."""
...
def hard_bits_from_syms(Xhat_matrix, Const, m, PowersOfTwo):
    """Hard demap to code bits (pre-decoder BER)."""
...
No = 1e-5
IsiDuration = 8
EbNoDB = np.arange(0, 3, 3).astype(np.int32)
...
# --------------------
N = 512                 # subcarriers
m = 4                   # bits per QAM symbol (16-QAM)
m_pilot = 4
NumOfdmSymbols = 40
...
# --------------------
USE_LDPC     = True
LDPC_dv      = 4        # variable node degree
LDPC_dc      = 8        # check node degree (must divide n_code)
LDPC_MAXITER = 100      # increased
LLR_SCALE    = 1.5      # slightly stronger LLRs
LLR_CLIP     = 20.0     # clip |LLR|

# Build LDPC (one codeword per OFDM symbol & TX)
if USE_LDPC:
    n_code = N * m
    if n_code % LDPC_dc != 0:
        raise ValueError(f"LDPC_dc={LDPC_dc} must divide n_code={n_code}. "
                         f"Try a DC that divides {n_code} (e.g., 8 for 2048).")
...
# Holders
# --------------------
# Pre-decoder BER (code bits when LDPC is enabled)
...
# Post-LDPC BER (info bits) if LDPC is enabled
...
for jj, ebno_db in enumerate(EbNoDB):
    ...
    # adaptive LDPC iters: use more at low SNR
    ...
    for kk in range(1, NumOfdmSymbols+1):
        if (np.remainder(kk, L) == 1):
            ...
            # Capacity, rank, conditioning (true H)
            ranks = []
            conds = []
            gamma = (Pi[jj]/No) / N_t
            cap_k = []
            ...
            # FFT to subcarrier domain (for linear equalizers only)
            ...
            # --------- Post-LDPC BER (info bits), if enabled ---------
            if USE_LDPC:
                snr_for_ldpc = 1.0  # we feed LLR/2 as y

                def decode_stream_llr(Xhat_col, dec_maxiter):
                    # Use empirical sigma^2 per stream for better LLR calibration
                    ...
                    y_obs = llr_vec / 2.0
                    d_hat = ldpc_decode(H, y_obs, snr_for_ldpc, maxiter=dec_maxiter)  # code bits
                    u_hat = get_message(G, d_hat).astype(np.int8)                      # info bits
                    return u_hat
...
# -------- Plots: SAVE + SHOW --------
# Pre-decoder BER (uncoded / pre-LDPC) — compares bit errors BEFORE LDPC decoding
plt.figure()
...
plt.title('4x8 MIMO | Pre-LDPC BER (code bits) | Nonlinear PA + Block Fading')
plt.xlabel('E_b/N_0 [dB]'); plt.ylabel('Bit Error Rate')
plt.tight_layout()
plt.savefig(f"{outdir}/BER_preLDPC_4x8.png", dpi=150)
plt.show()

# Post-decoder BER (coded / post-LDPC) — compares info-bit errors AFTER LDPC decoding
if USE_LDPC:
    plt.figure()
    ...
    plt.title('4x8 MIMO | Post-LDPC BER (rate≈1/2 LDPC)')
    plt.tight_layout()
    plt.savefig(f"{outdir}/BER_postLDPC_4x8.png", dpi=150)
    plt.show()

# Combined figure (optional): overlay pre- and post-LDPC ESN curves only
if USE_LDPC:
    plt.figure()
    plt.semilogy(EbNoDB, BER_ESN_matched, 'gd--', label='ESN matched (pre-LDPC)')
    ...
    plt.grid(True, which='both', ls=':'); plt.legend()
    plt.xlabel('E_b/N_0 [dB]'); plt.ylabel('Bit Error Rate')
    plt.title('ESN: Pre- vs Post-LDPC BER')
    plt.tight_layout()
    plt.savefig(f"{outdir}/BER_ESN_pre_vs_postLDPC_4x8.png", dpi=150)
    plt.show()

print("Saved figures to:")
print(f" - {outdir}/BER_preLDPC_4x8.png")
if USE_LDPC:
    print(f" - {outdir}/BER_postLDPC_4x8.png")
    print(f" - {outdir}/BER_ESN_pre_vs_postLDPC_4x8.png")
print(f" - {outdir}/channel_metrics_mimo4x8.pkl")
