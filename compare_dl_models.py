...
Compare ESN vs. ELM, FNN (MLP), CNN (1D), and RNN (LSTM) receivers on a 2x2 OFDM MIMO
nonlinear block-fading channel. Reuses HelpFunc.trainMIMOESN to generate supervised
input/output pairs, then evaluates BER across Eb/N0 values. Saves results and plots.

Usage:
    python compare_dl_models.py

Outputs:
    - BER_compare.pkl  (dict with EBN0 and BER arrays per model)
    - BER_compare.png  (plot of BER vs. Eb/N0)
    - (optional) NMSE_compare.pkl, NMSE_compare.png

Notes:
    - Requires TensorFlow/Keras for FNN/CNN/RNN.
    - ELM is implemented from scratch (no extra deps).
    - Assumes HelpFunc.py and pyESN.py are in PYTHONPATH (same folder).
"""
...
class ELM:
    """Simple Extreme Learning Machine (single hidden layer, ridge regression readout)."""
    def __init__(self, n_hidden=256, activation="tanh", alpha=1e-3, seed=42):
...
def build_cnn(time_steps, feat_dim, output_dim):
    import tensorflow as tf
    from tensorflow import keras
    from tensorflow.keras import layers
...
def build_rnn(time_steps, feat_dim, output_dim):
    import tensorflow as tf
    from tensorflow import keras
    from tensorflow.keras import layers
...
def get_default_params():
    ...
    # SNR sweep
    params["EbNoDB"] = np.arange(0, 10 + 1, 3).astype(np.int32)
...
def unit_qam_constellation(m):
    """Normalized QAM constellation of order 2^m (same as HelpFunc.UnitQamConstellation)."""
    return HelpFunc.UnitQamConstellation(m)


def bits_to_constellation_idx(bits_row):
    """Convert bit vector (LSB first) to decimal index."""
    # bits_row expected shape (m,)
    idx = 0
    for i, b in enumerate(bits_row):
        idx += (2**i) * int(b)
    return idx
    ...
    for jj, eb in enumerate(EbNoDB):
        ...
        for kk in range(1, NumOfdmSymbols + 1):
            ...
            if (np.remainder(kk, L) == 1):
                ...
                # Prepare arrays for other models (drop the initial forget points)
                X_tr = ESN_input[nForgetPoints:, :]
                Y_tr = ESN_output[nForgetPoints:, :]

                # Fit ELM
                elm = ELM(n_hidden=256, activation="tanh", alpha=1e-3, seed=42)
                elm.fit(X_tr, Y_tr)
                ...
                try:
                    ...
                    # MLP
                    mlp = build_mlp(input_dim=X_tr.shape[1], output_dim=Y_tr.shape[1])
                    mlp.fit(X_tr, Y_tr, epochs=5, batch_size=1024, verbose=0)

                    # CNN/RNN expect 3D input (batch, time, feat)
                    X_tr_seq = X_tr[None, :, :]       # (1, T, F)
                    Y_tr_seq = Y_tr[None, :, :]       # (1, T, O)

                    # CNN
                    cnn = build_cnn(time_steps=X_tr.shape[0], feat_dim=X_tr.shape[1], output_dim=Y_tr.shape[1])
                    cnn.fit(X_tr_seq, Y_tr_seq, epochs=5, batch_size=1, verbose=0)

                    # RNN
                    rnn = build_rnn(time_steps=X_tr.shape[0], feat_dim=X_tr.shape[1], output_dim=Y_tr.shape[1])
                    rnn.fit(X_tr_seq, Y_tr_seq, epochs=5, batch_size=1, verbose=0)
                ...
                # --- ELM / FNN / CNN / RNN predictions ---
                def predict_and_build_xhat(model, kind):
                    if model is None:
                        return None
                    if kind in ["MLP", "ELM"]:
                        Y_pred = model.predict(ESN_input_test[nForgetPoints:, :])
                    else:
                        X_seq = ESN_input_test[nForgetPoints:, :][None, :, :]
                    ...
                    Xhat = np.zeros_like(X, dtype=np.complex128)
                    xhat_td = np.hstack((x0.reshape(-1,1), x1.reshape(-1,1)))
                    # NMSE
                    NMSE_tmp = (
                        np.linalg.norm(xhat_td[:,0] - x[:,0])**2 / np.linalg.norm(x[:,0])**2
                        + np.linalg.norm(xhat_td[:,1] - x[:,1])**2 / np.linalg.norm(x[:,1])**2
                    )
                    for ii in range(N_t):
                ...
                # Pilot structure for LS interpolation:
                # Insert zeros at pilot locations for the other antenna
                X_LS = np.copy(X)
                ...
                X_hat_Perfect = np.zeros_like(X, dtype=np.complex128)
                X_hat_LS = np.zeros_like(X, dtype=np.complex128)
                X_hat_MMSE = np.zeros_like(X, dtype=np.complex128)
                for ii in range(N):
                    Y_temp = Y_NLD[ii, :].T
        ...
        # Average NMSE over the number of data symbols evaluated
        # (We divided by NMSE_count in the baseline; here we approximate by the count of data symbols)
        # For simplicity, we normalize by (NumOfdmSymbols - number of pilot symbols)
...
def parse_args():
    ...
    # In case you want to override defaults via CLI later
